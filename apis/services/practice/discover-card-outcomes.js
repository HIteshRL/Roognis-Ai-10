 ...
 * coupling from practice back to Discover, and it only ever reads.
 *
 ...
 * of 4000 used by services/ai when it calls into practice. This call is
 * nested inside a request that ai itself is timing out at 4000ms, so a slow
 * Discover must not push practice's own latency into ai's timeout budget.
 *
 * FAILS SOFT, ALWAYS: any error, timeout, or malformed payload yields [], and
 * practice's learning context is still returned without card evidence.
...
async function fetchCardOutcomes({
  ...
  try {
    ...
    console.warn('[practice] card attempt outcomes unavailable:', err.message);
    return [];
  }
}
