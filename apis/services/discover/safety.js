 ...
 *
 * services/discover has no student-authored-prompt path (proposals are derived
 * from articles the student already read) and no image path, so this keeps only
 * what's used: a check over generated search queries and proposed interest
 * labels before either is persisted, and over the title/summary of any article
 * a hunt brings back off the open web. The rule set itself is copied verbatim, not
